...
(function () {
  ...
  var I18N = {
    ...
    sub_terms_intro: { zh: '欢迎使用 Subfolks（"本应用"）。使用本应用即表示您同意本协议条款。', ja: 'Subfolks（「本アプリ」）をご利用いただきありがとうございます。利用をもって本規約に同意したものとみなされます。', en: 'Welcome to Subfolks ("the App"). By using the App you agree to these Terms.' },
    sub_terms_s1_title: { zh: '1. 协议适用范围与同意', ja: '1. 規約の適用と同意', en: '1. Scope & Agreement' },
    sub_terms_s1_body: { zh: '本协议适用于您对 Subfolks 的下载、安装与使用。下载、安装或使用即表示您已阅读并同意本协议。若不同意，请勿使用本应用。', ja: '本規約は Subfolks のダウンロード・インストール・利用に適用されます。利用をもって読了・同意したものとみなされます。同意できない場合は利用しないでください。', en: 'These Terms apply to your download, installation, and use of Subfolks. By using the App you agree to be bound by them. If you do not agree, please do not use the App.' },
    sub_terms_s2_title: { zh: '2. 服务说明', ja: '2. サービスの説明', en: '2. Service Description' },
    sub_terms_s2_body: { zh: 'Subfolks 帮助您和家人记录并管理订阅服务：录入订阅与成员、到期/试用提醒、按真实货币记账、小组件展示、iCloud 同步（可选）与统计。', ja: 'Subfolks は、サブスクの記録と管理を家族とともに行うアプリです。登録・更新リマインダー・通貨別記録・ウィジェット・iCloud 同期（任意）・統計を提供します。', en: 'Subfolks helps you and your family record and manage subscriptions: entries and members, renewal reminders, real-currency records, widgets, optional iCloud sync, and insights.' },
    sub_terms_s3_title: { zh: '3. 账号与数据', ja: '3. アカウントとデータ', en: '3. Accounts & Data' },
    sub_terms_s3_body: { zh: '本应用无需注册账号。您的订阅记录与成员信息默认仅保存在本机；若您开启 iCloud 同步，则经 Apple iCloud 加密存储并仅在您授权的设备间同步。', ja: '本アプリはアカウント不要です。サブスクとメンバーの記録はデフォルトで端末内にのみ保存されます。iCloud 同期を有効にした場合のみ、Apple iCloud 経由でご自身の端末間で同期されます。', en: 'The App requires no account. Records are stored on your device by default. If you enable iCloud sync, data is encrypted and synced by Apple iCloud across devices you authorize.' },
    sub_terms_s4_title: { zh: '4. Ultra 一次性买断', ja: '4. Ultra 買い切り', en: '4. Ultra One-time Purchase' },
    sub_terms_s4_body: { zh: 'Ultra 为一次性买断（Non-Consumable），无自动续订。购买与恢复购买均由 App Store 处理，我们不会获取或存储您的支付信息。', ja: 'Ultra は自動更新のない買い切り（Non-Consumable）です。購入・購入の復元は App Store が処理し、当方は支払い情報を取得・保存しません。', en: 'Ultra is a one-time (Non-Consumable) purchase with no auto-renewal. Purchases and restore are handled by the App Store; we never see your payment details.' },
    sub_terms_s5_title: { zh: '5. 知识产权', ja: '5. 知的財産権', en: '5. Intellectual Property' },
    sub_terms_s5_body: { zh: '本应用的软件、界面、图标与内容受著作权保护。您不得复制、修改、逆向工程或分发。您录入的数据归您所有。', ja: '本アプリのソフトウェア・UI・アイコン・コンテンツは著作権で保護されています。複製・改変・リバースエンジニアリング・配布はできません。入力したデータはお客様のものです。', en: 'The App software, interface, icons, and content are protected. You may not copy, modify, reverse-engineer, or distribute them. Data you enter belongs to you.' },
    sub_terms_s6_title: { zh: '6. 可接受使用', ja: '6. 許容される利用', en: '6. Acceptable Use' },
    sub_terms_s6_body: { zh: '请仅以合法目的使用本应用，不得用于违法、侵权或骚扰等活动。', ja: '本アプリは合法的な目的でのみ使用してください。違法・権利侵害・ハラスメント等に使用しないでください。', en: 'Use the App only for lawful purposes. Do not use it for illegal, infringing, or harassing activity.' },
    sub_terms_s7_title: { zh: '7. 免责声明与责任限制', ja: '7. 免責と責任の制限', en: '7. Disclaimers & Limitation' },
    sub_terms_s7_body: { zh: '提醒功能仅为辅助，最终扣费以服务方为准。本应用按"现状"提供，在法律允许的范围内，我们不对因使用本应用造成的间接损失承担责任。', ja: '通知は補助機能であり、最終的な引き落としは各サービスの規定に従います。本アプリは「現状有姿」で提供され、法令の許す範囲で、利用により生じた間接損害について責任を負いません。', en: 'Reminders are assistance only; actual charges follow each provider. The App is provided "as is"; to the extent permitted by law, we are not liable for indirect damages from its use.' },
    sub_terms_s8_title: { zh: '8. 变更与联系', ja: '8. 変更とお問い合わせ', en: '8. Changes & Contact' },
    sub_terms_s8_body: { zh: '我们可能更新本协议并在应用内通知。如有疑问请联系：support@subfolks.app', ja: '本規約は随時更新され、重要な変更はアプリ内でお知らせします。ご質問は support@subfolks.app まで。', en: 'We may update these Terms and will notify material changes in-app. Questions: support@subfolks.app' },

    // ---- 隐私政策（Subfolks）----
    sub_privacy_title: { zh: '隐私政策', ja: 'プライバシーポリシー', en: 'Privacy Policy' },
    sub_privacy_date: { zh: '2026 年 9 月', ja: '2026年9月', en: 'September 2026' },
    sub_privacy_intro: { zh: '我们非常重视您的隐私。本政策说明 Subfolks 如何处理您的数据。', ja: 'お客様のプライバシーを重視しています。Subfolks のデータの取り扱いを説明します。', en: 'We take your privacy seriously. This policy explains how Subfolks handles your data.' },
    sub_privacy_s1_title: { zh: '1. 数据控制者与本地优先', ja: '1. 管理者とローカル優先', en: '1. Controller & Local-First' },
    sub_privacy_s1_body: { zh: 'Subfolks 不设服务器。您的订阅记录、成员信息等数据默认只保存在您的设备本地，我们无法访问。', ja: 'Subfolks はサーバーを持ちません。サブスクやメンバーの記録はデフォルトで端末内にのみ保存され、当方からはアクセスできません。', en: 'Subfolks has no servers. Your records stay on your device by default and are not accessible to us.' },
    sub_privacy_s2_title: { zh: '2. 我们收集的数据', ja: '2. 収集するデータ', en: '2. Data We Collect' },
    sub_privacy_s2_body: { zh: '我们不收集、不上传、不出售您的任何订阅数据。本应用不含第三方广告与分析追踪器。', ja: '当方はサブスクデータを収集・送信・販売しません。本アプリは第三者広告や分析トラッカーを含みません。', en: 'We do not collect, upload, or sell your subscription data. The App has no third-party ads or analytics trackers.' },
    sub_privacy_s3_title: { zh: '3. iCloud 同步', ja: '3. iCloud 同期', en: '3. iCloud Sync' },
    sub_privacy_s3_body: { zh: '仅当您主动开启 iCloud 同步时，数据会经 Apple iCloud（私有数据库）在您授权的设备间同步。首次同步若两端都有数据，我们会让您选择合并方式。', ja: 'お客様が iCloud 同期を有効にした場合のみ、Apple iCloud（プライベートデータベース）経由でご自身の端末間で同期されます。初回に両方にデータがある場合は、統合方法をお選びいただきます。', en: 'Only when you enable iCloud sync is your data synced through Apple iCloud (private database) across devices you authorize. If both sides have data at first sync, we ask how you\'d like to combine it.' },
    sub_privacy_s4_title: { zh: '4. 通知与提醒', ja: '4. 通知とリマインダー', en: '4. Notifications' },
    sub_privacy_s4_body: { zh: '到期提醒使用 iOS 本地通知，在您的设备上生成，不会上传到任何服务器。', ja: '更新リマインダーは iOS ローカル通知として端末上で生成され、サーバーへ送信されません。', en: 'Reminders are scheduled as iOS local notifications on your device and are never uploaded.' },
    sub_privacy_s5_title: { zh: '5. 第三方服务', ja: '5. 第三者サービス', en: '5. Third-Party Services' },
    sub_privacy_s5_body: { zh: '使用的第三方服务包括：Apple（App Store 购买与 iCloud）、公开汇率数据源（仅查询目标货币代码）。我们未接入广告或第三方分析 SDK。', ja: '利用する第三者サービスは Apple（App Store 購入・iCloud）と公開為替データ源（通貨コードの照会のみ）です。広告・第三者分析 SDK はありません。', en: 'Third-party services used: Apple (App Store purchases and iCloud) and a public exchange-rate source (querying currency codes only). No ad or analytics SDKs.' },
    sub_privacy_s6_title: { zh: '6. 数据保留与删除', ja: '6. データの保持と削除', en: '6. Retention & Deletion' },
    sub_privacy_s6_body: { zh: '数据保留在您的设备（及您开启的 iCloud）中。您可随时在应用内删除任何记录；删除应用即删除本地数据。', ja: 'データは端末（および有効にした iCloud）に保持されます。いつでもアプリ内で削除できます。アプリを削除するとローカルデータも削除されます。', en: 'Data is kept on your device (and iCloud if enabled). You can delete any record at any time; deleting the App removes local data.' },
    sub_privacy_s7_title: { zh: '7. 您的权利', ja: '7. お客様の権利', en: '7. Your Rights' },
    sub_privacy_s7_body: { zh: '依据适用隐私法律（GDPR、日本与中国个人信息保护法），您可以访问、更正、删除数据。由于数据在本机，您可直接在应用内完成。', ja: '適用される法律（GDPR、日本の個人情報保護法、中国のPIPL）に基づき、アクセス・訂正・削除の権利があります。データは端末内にあるため、アプリ内で直接行えます。', en: 'Under applicable privacy laws you may access, correct, or delete your data. Since data is on your device, you can do so directly in the App.' },
    sub_privacy_s8_title: { zh: '8. 儿童隐私', ja: '8. 子どものプライバシー', en: '8. Children\'s Privacy' },
    sub_privacy_s8_body: { zh: '本应用面向订阅管理使用场景，不针对儿童设计，也不会故意收集儿童个人数据。', ja: '本アプリはサブスク管理を目的とし、子ども向けではなく、子どもの個人データを意図的に収集しません。', en: 'The App is intended for subscription management, is not designed for children, and we do not knowingly collect children\'s data.' },
    sub_privacy_s9_title: { zh: '9. 变更与联系', ja: '9. 変更とお問い合わせ', en: '9. Changes & Contact' },
    sub_privacy_s9_body: { zh: '本政策可能更新并会标注日期。如有疑问请联系：support@subfolks.app', ja: '本ポリシーは更新されることがあり、日付が記載されます。ご質問は support@subfolks.app まで。', en: 'This policy may be updated and carries its revision date. Questions: support@subfolks.app' },
