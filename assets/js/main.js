...
(function () {
  ...
  var I18N = {
    ...
    sub_footer_tm: { zh: 'App 内出现的产品名称、商标与标志均归其各自所有者所有，仅用于帮助您识别所管理的订阅服务，不代表相关品牌对本应用的认可、赞助或关联。', ja: '本アプリに表示される製品名・商標・ロゴは、それぞれの権利者に帰属します。これらはご利用中のサブスクリプションを識別するためにのみ使用され、当該ブランドによる本アプリの推薦・後援・提携を示すものではありません。', en: 'All product names, trademarks, and logos shown in the App belong to their respective owners and are used solely to help you identify the subscriptions you manage. They do not imply endorsement, sponsorship, or affiliation.' },
    ...
    sub_terms_s9_title: { zh: '9. 第三方品牌与服务标识', ja: '9. 第三者のブランド・商標', en: '9. Third-Party Brands & Trademarks' },
    sub_terms_s9_body: { zh: '本应用可能展示第三方服务商（如流媒体、音乐、云服务等）的产品名称、图标与标志，上述名称与标识均归其各自权利人所有。我们展示这些标识，仅用于帮助您识别和录入您实际使用的订阅服务，不代表您因此获得该等服务的授权，也不代表相关品牌对本应用有任何认可、赞助或关联关系。本应用与上述品牌相互独立；您在本应用内创建的任何记录均由您自行管理与负责。', ja: '本アプリには、動画・音楽・クラウドなどの第三者サービスの製品名・アイコン・ロゴが表示されることがあります。これらの名称・商標・ロゴは、それぞれの権利者に帰属します。表示は、お客様が実際に利用しているサブスクリプションを識別・登録するためのみに用いられ、当該サービスの利用権を付与するものではなく、当該ブランドが本アプリを推薦・後援・提携していることを示すものではありません。本アプリは上記ブランドとは独立しており、本アプリ内で作成した記録はお客様ご自身が管理・責任を負います。', en: 'The App may display product names, icons, and logos of third-party services (such as streaming, music, and cloud services). Such names and marks are the property of their respective owners. They appear solely to help you identify and record the subscriptions you actually use, do not grant you access to those services, and do not imply endorsement, sponsorship, or affiliation by any of those brands. The App is independent of those brands, and any records you create in the App are yours to manage.' },
    sub_terms_s10_title: { zh: '10. 可接受使用', ja: '10. 許容される利用', en: '10. Acceptable Use' },
    sub_terms_s10_body: { zh: '您同意仅以合法、正当目的使用本应用，不得利用本应用从事任何违法、侵权、欺诈、骚扰或损害他人权益的活动，也不得试图规避或破坏本应用的任何技术限制。', ja: 'お客様は本アプリを合法的かつ正当な目的でのみ使用することに同意します。違法行為・権利侵害・詐欺・ハラスメント・他人の権利を害する行為、および技術的制限の回避・破壊を試みてはなりません。', en: 'You agree to use the App only for lawful and legitimate purposes and not to use it for any illegal, infringing, fraudulent, harassing, or harmful activity, nor attempt to circumvent or disrupt any technical limitation of the App.' },
    sub_terms_s11_title: { zh: '11. 免责声明与责任限制', ja: '11. 免責事項と責任の制限', en: '11. Disclaimers & Limitation of Liability' },
