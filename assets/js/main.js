...
(function () {
  ...
  var I18N = {
    ...
    terms_s1_body: { zh: '本协议（"协议"）适用于您（"您"）对 SpeechPrompter 应用（"本应用"）的下载、安装与使用。下载、安装或使用本应用，即表示您确认已阅读、理解并同意受本协议约束。若您不同意本协议，请勿下载或使用本应用。', ja: '本規約（「本規約」）は、SpeechPrompter アプリ（「本アプリ」）のダウンロード・インストール・利用に適用されます。本アプリをダウンロード・インストール・利用した時点で、本規約を読了し、理解し、同意したものとみなされます。本規約に同意できない場合は、本アプリをダウンロード・利用しないでください。', en: 'These Terms of Service ("Terms") apply to your ("you") download, installation, and use of the SpeechPrompter application ("the App"). By downloading, installing, or using the App, you acknowledge that you have read, understood, and agree to be bound by these Terms. If you do not agree, please do not download or use the App.' },
    ...
    privacy_s1_body: { zh: '本隐私政策说明 SpeechPrompter（"我们"）如何处理您的数据。我们是本应用的数据控制者。我们遵循"数据最小化"原则，仅处理提供服务所必需的数据。', ja: '本プライバシーポリシーは、SpeechPrompter（「当方」）がお客様のデータをどのように取り扱うかを説明します。当方は本アプリのデータ管理者です。「データ最小化」の原則に従い、サービス提供に必要なデータのみを処理します。', en: 'This Privacy Policy explains how SpeechPrompter ("we") handles your data. We are the data controller for the App. We follow the principle of data minimization, processing only what is necessary to provide the service.' },
